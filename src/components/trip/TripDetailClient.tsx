...
export default function TripDetailClient({ trip, userId }: TripDetailClientProps) {
  ...
  // 获取日期范围
  const days = getDaysRange(localTrip.start_date, localTrip.end_date);
    ...
    <div className="min-h-screen bg-background text-ink-800 pb-24 md:pb-12">
          ...
          <div className="flex items-center gap-2">
            ...
            {isCreator && (
              <button
                ...
                className="p-2 hover:bg-white/50 rounded-full transition-colors text-ink-600 hover:text-primary-600 touch-target"
                title="编辑行程"
              >
                <Edit className="w-5 h-5" />
              </button>
            )}
            <ShareButton tripId={localTrip.id} shareCode={localTrip.share_code} tripName={localTrip.name} creatorName={creatorName} />
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 relative z-10">
          ...
          <div className="relative z-10">
            <div className="flex flex-wrap items-center gap-6 text-sm text-ink-600 mb-4">
              <div className="flex items-center gap-2 bg-white/50 px-3 py-1.5 rounded-full">
                ...
                <span>
                  {localTrip.start_date} 至 {localTrip.end_date}
                </span>
              </div>
              <div className="flex items-center gap-2 bg-white/50 px-3 py-1.5 rounded-full">
                <Users className="w-4 h-4 text-primary-500" />
                <span>{localTrip.trip_members?.length || 0} 位旅行家</span>
              </div>
              <div className="flex items-center gap-2 bg-white/50 px-3 py-1.5 rounded-full">
            ...
            {localTrip.description && (
              <p className="text-ink-700 leading-relaxed max-w-2xl">{localTrip.description}</p>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4">
            {days.map((day, index) => {
              ...
              return (
                <Link
                  key={dateStr}
                  href={`/trips/${localTrip.id}/calendar?date=${dateStr}`}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <Link href={`/trips/${localTrip.id}/calendar`} className="group">
          ...
          <Link href={`/trips/${localTrip.id}/map`} className="group">
            <div className="glass-card rounded-2xl p-6 hover:shadow-float hover:-translate-y-1 transition-all h-full">
          ...
          <Link href={`/trips/${localTrip.id}/gallery`} className="group">
            <div className="glass-card rounded-2xl p-6 hover:shadow-float hover:-translate-y-1 transition-all h-full">
          ...
          <Link href={`/trips/${localTrip.id}/members`} className="group">
            <div className="glass-card rounded-2xl p-6 hover:shadow-float hover:-translate-y-1 transition-all h-full">
      ...
      <nav className="md:hidden fixed bottom-6 left-6 right-6 h-16 glass-nav rounded-2xl shadow-float flex justify-around items-center px-2 z-40 border border-white/50">
        <div className="flex justify-around w-full">
          <Link href={`/trips/${localTrip.id}/calendar`} className="flex flex-col items-center p-2 text-ink-500 hover:text-primary-500 transition-colors">
            <Calendar className="w-6 h-6" strokeWidth={1.5} />
            <span className="text-[10px] font-medium mt-1">日历</span>
          </Link>
          <Link href={`/trips/${localTrip.id}/map`} className="flex flex-col items-center p-2 text-ink-500 hover:text-primary-500 transition-colors">
            <Navigation className="w-6 h-6" strokeWidth={1.5} />
            <span className="text-[10px] font-medium mt-1">地图</span>
          </Link>
          <Link href={`/trips/${localTrip.id}/logs`} className="flex flex-col items-center p-2 text-ink-500 hover:text-primary-500 transition-colors">
            <MapPin className="w-6 h-6" strokeWidth={1.5} />
            <span className="text-[10px] font-medium mt-1">记录</span>
          </Link>
          <Link href={`/trips/${localTrip.id}/gallery`} className="flex flex-col items-center p-2 text-ink-500 hover:text-primary-500 transition-colors">
      ...
      {showEditModal && (
        <EditTripModal
          tripId={localTrip.id}
          initialName={localTrip.name}
          initialDescription={localTrip.description}
          initialStartDate={localTrip.start_date}
          initialEndDate={localTrip.end_date}
          initialCoverUrl={localTrip.cover_image_url}
